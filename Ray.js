function Ray(x, y, direction) {
  ...
  this.hitWallY = function (x, y) {
    ...
    // if(map[yBlock * gridSize + xBlock] === 1) console.log("Y", xBlock, yBlock)
  ...
  this.hitWallX = function (x, y) {
    ...
    // if(map[yBlock * gridSize + xBlock] === 1) console.log("X", xBlock, yBlock)
    let isWall = map[yBlock * gridSize + xBlock] === 1
    return map[yBlock * gridSize + xBlock] === 1
  }
  this.getNextXIntersect = function (lastXIntersect, xStep, isUp) {
    ...
    newXIntercept.dist = Math.sqrt(Math.pow(player.x - newXIntercept.x - lastXIntersect.x, 2) + Math.pow(player.y - newXIntercept.y - lastXIntersect.y, 2))
    // newXIntercept.dist = 99999999
    return newXIntercept
  ...
  this.getNextYIntersect = function (lastYIntersect, yStep, isLeft) {
    ...
    newYIntercept.dist = Math.sqrt(Math.pow(player.x - newYIntercept.x - lastYIntersect.x, 2) + Math.pow(player.y - newYIntercept.y - lastYIntersect.y, 2))
    return newYIntercept
  }
    ...
    while (coords === null) {
      ...
      if (nextXIntersect.dist < nextYIntersect.dist) {
        if (this.hitWallX(nextXIntersect.x, nextXIntersect.y)) {
          ...
          if (i > 50) {
            coords = [0, 0]; //console.log("BlockX", Math.floor(x / blockSize) + (player.x > x ? -1 : 0))
          }
        ...
        if (this.hitWallY(nextYIntersect.x, nextYIntersect.y)) {
          ...
          if (i > 50) {
            coords = [0, 0]; //console.log("BlockY", Math.floor(y / blockSize) + (player.y > y ? -1 : 0))
          }
      ...
      //nextXIntersect = this.getNextXIntersect(xIntersect, xStep, isLeft, isUp)
